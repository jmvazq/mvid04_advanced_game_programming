...
std::string projectDir = "../tests/AG_11_02";
...
void render(const uint32_t& VAO, const Shader& shader_bump, const Shader& shader_spec, const int32_t text_dif, const uint32_t text_spec, const int32_t text_norm) {
	...
	float l_pos[] = { std::sin((float)glfwGetTime() / 2.0f) * 10.0f, 0.0f, std::abs(std::cos((float)glfwGetTime() / 2.0f)) };
	...
	// Second quad
	{
		...
		shader_spec.set("projection", proj);
...
int main(int argc, char* argv[])
{
	...
	const Shader shader_spec((projectDir + "/cube.vs").c_str(), (projectDir + "/cube.fs").c_str());
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Render
		render(VAO, shader_bump, shader_spec, text_dif, text_spec, text_normal);
