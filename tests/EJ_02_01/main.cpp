...
void render(uint32_t VAO, uint32_t program) {
	...
	glDrawArrays(GL_TRIANGLES, 0, 6); // use this to draw when not using element buffer objects (indexed drawing)
}
...
int main(int argc, char* argv[])
{
	...
	uint32_t VAO = createVertexData(&VBO); // vertex array object
