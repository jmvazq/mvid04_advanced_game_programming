...
std::string projectDir = "../tests/AG_10_01";
...
void render(const uint32_t& cubeVAO, const uint32_t& quadVAO, const Shader& shader, const int32_t text_dif, const uint32_t text_spec) {
	glClearColor(0.0f / 255.0f, 0.0f / 255.0f, 0.0f / 255.0f, 1.0f);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	shader.use();
	shader.set("objectColor", 1.0f, 0.5f, 0.31f);
...
int main(int argc, char* argv[])
{
	...
	const Shader shader((projectDir + "/cube.vs").c_str(), (projectDir + "/cube.fs").c_str());
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Render
		render(cubeVAO, quadVAO, shader, text_dif, text_spec);
